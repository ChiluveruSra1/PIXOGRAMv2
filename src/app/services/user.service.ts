...
export class UserService {
  private baseUrl = 'http://localhost:8052/user';

  private followUrl='http://localhost:8050/follow'
  ...
  getFollowing(userid: number): Observable<Object>{
    return this.http.get(`http://localhost:8050/follow/following/`+`${userid}`)
  }

  getFollower(userid: number): Observable<Object>{
    return this.http.get(`http://localhost:8050/follow/follower/`+`${userid}`)
  }
